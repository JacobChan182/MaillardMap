...
- **`PUBLIC_EMAIL_CONFIRM_WEB_URL`** (optional): public base URL of your **marketing/support site** (e.g. `https://maillardmap.web.app`) **with no trailing slash**. When set, signup confirmation links use `/verify-email?token=...` on this host (the page calls `GET /auth/verify-email` on the API). If unset, links use `${PUBLIC_API_BASE_URL}/auth/verify-email?token=...`. When this variable **is** set, `GET /auth/verify-email` still **302-redirects** a normal browser tab (HTML `Accept`) to the same `/verify-email?token=...` on the web app so older emails that pointed at the API still open the Firebase page.
